/**
 * Proxy POST /api/approvals/approve to Laravel so the request body is forwarded.
 * Next.js rewrites can fail to forward POST bodies; this route ensures the payload reaches the backend.
 * Body: { id: number, comments?: string }
 */
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const url = `${BACKEND_URL.replace(/\/$/, "")}/approvals/${id}/approve`;
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ comments: body.comments }),
    });
