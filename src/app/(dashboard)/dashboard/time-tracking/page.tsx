...
async function exportCertificateAsPdf(data: CertificateData): Promise<void> {
  ...
  doc.text(`This is to certify that ${data.userName} has successfully completed the required hours`, margin, y, { maxWidth: pageW - 2 * margin });
  y += 8;
  doc.text(`for the ${data.programName} program under the ${APP_CONFIG.name}.`, margin, y, { maxWidth: pageW - 2 * margin });
  y += 14;
  doc.setFont("helvetica", "bold");
  ...
  doc.text("This certificate was generated by the PESO OJT Attendance System and is valid for official records.", pageW / 2, y, { align: "center", maxWidth: pageW - 2 * margin });
  y += 12;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Dialog open={certificateOpen} onOpenChange={setCertificateOpen}>
        <DialogContent onClose={() => setCertificateOpen(false)} className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-center">Certificate of Completion</DialogTitle>
          </DialogHeader>
          {certificateRow && (
            <>
              <div className="rounded-lg border border-slate-200 bg-slate-50/50 px-4 py-4 text-sm text-slate-700">
                <p className="mb-3 text-center text-xs font-semibold uppercase tracking-wide text-slate-500">{APP_CONFIG.name}</p>
                <p className="mb-4 text-center text-lg font-semibold text-slate-900">Certificate of Completion</p>
                <p className="mb-4 text-slate-600">
                  This is to certify that <span className="font-semibold text-slate-900">{certificateRow.name}</span> has successfully completed the required hours for the{" "}
                  <span className="font-semibold text-slate-900">{internMap.get(certificateRow.intern_id)?.course ?? "Internship"}</span> program.
                </p>
                <dl className="space-y-2 border-t border-slate-200 pt-3">
                  ...
                  <div className="flex justify-between gap-2">
                    ...
                    <dd className="font-medium text-slate-900">{internMap.get(certificateRow.intern_id)?.course ?? "Internship"}</dd>
                  </div>
                  <div className="flex justify-between gap-2">
                    <dt className="text-slate-500">Total Rendered Time</dt>
                    <dd className="font-medium text-slate-900 tabular-nums">{formatHours(certificateRow.hours_rendered)}</dd>
                  </div>
                  <div className="flex justify-between gap-2">
                    <dt className="text-slate-500">Completion Date</dt>
                    <dd className="font-medium text-slate-900">{buildCertificateData(certificateRow, internMap.get(certificateRow.intern_id)).completionDate}</dd>
                  </div>
                  <div className="flex justify-between gap-2">
              ...
              <DialogFooter className="gap-2 sm:gap-0">
                <Button type="button" variant="outline" onClick={() => setCertificateOpen(false)}>Close</Button>
                <Button
                  type="button"
                  onClick={() => {
